...
**Branch `cta-sampling`:** CTA-level sampling to speed up simulation. Foundation (sampling + classifier + pilot loop), Phase A (refined classifier inputs), Phase B (whole-kernel cycle estimator), wider-validation gap fixes (C1 tiny-grid skip, C2 K-rep replication fix), and a configurable concurrency-throughput model (`-cta_sampling_concurrency_model {logfit, sat_exp, roofline_clamp, roofline_exp}`, default `sat_exp`) are implemented and validated. Cycle accuracy: p50 9.5% / p90 23.4% / max 30.0% on the wider 9-workload set; cumulative wall-time speedup 1.05× on rodinia2 toy traces (production-scale projected 100×+, unmeasured). Start with `HANDOFF.md` for the current self-contained state and next steps; `history/` holds the three walkthrough-derived progression records.
