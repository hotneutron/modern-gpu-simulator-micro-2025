...
- **Resolved**
  - **Phase/wait parity semantics fixed (was an FA3-bwd deadlock).** Diagnosed from run
    ...
    the parity bit). Confirmed in practice by the full FA3 run below: `wait miss` and
    `wait immediate-hit` now coexist in healthy proportions and phases cycle both
    directions.
  ...
  - **FA3 full-trace run confirms the sync model (functional).** The first post-fix full simulation
    (`...-53cb9e043fde.{o297,e297}`) ran `kernel-10` to completion with
    `GPGPU-Sim: *** exit detected ***`, no deadlock/abort/FATAL. Final results:
    - `gpu_sim_cycle = 336,579` vs the sync-skip baseline `...-17cf425686f3.e295`
      `gpu_sim_cycle = 339,429` → **−0.8%**. Adding real sync modeling did not blow up the
      cycle count, i.e. no spurious over-stall and no deadlock.
    - Both barrier roles cycle correctly: arrival-only count barriers flipped with
      `arrive=256/256` (**6,527** flips); tx (TMA-data) barriers flipped with tx bytes
      exactly filled (`tx=16896/16896` ×6,528, `tx=32768/32768` ×384). Aggregate
      `phase_flip=13,440`, `wait_pending=787,624`, `wait_released=631,692`,
      `arrive=52,224`, `arrive_expect_tx=6,912`, `tma_completions=20,856`.
    - `skip sync` is **only** predicated-off `EXCH` (4,224) and `PHASECHK` (3,958);
      **zero** `ARRIVE`/wait sites are dropped — the runtime-valid gate never discards a
      sync that contributes to barrier state.
  - **Caveats (timing not yet validated).** TMA data movement has no real latency yet
    (transfers complete effectively immediately), so consumers barely stall on tx barriers
    — that is *why* the cycle count matches the sync-skip baseline. This run proves the
    sync **state machine** is functionally correct (no deadlock, dependency order honored),
    but **sync timing/stall accuracy is still unverified** and must be re-checked once TMA
    acquires real latency. Also, only the wait-state values (`0x0`/`0x80000000`) and the 9
    barriers seen in *this* trace are exercised; **other kernels / mbarrier patterns are
    not yet validated**.
- **Status**
  - Sync is **functionally complete for the FA3 bf16 bwd-causal trace**: arrive/EXCH
    semantics (thread-unit arrivals + EXCH `/2`), phase-parity decode (bit 31) and wait
...
- Hopper mbarrier sync debug logging (SYNCDBG), **disabled by default**:
  - `-sync_debug_enable 0` — master switch; set to `1` to emit per-SM sync event logs to
    stderr (was previously hard-coded on). No rebuild needed to toggle.
  - `-sync_debug_print_budget 20000000` — per-SM cap on sync event log lines.
  - `-sync_debug_skip_runtime_budget 1024` — per-SM cap on `skip sync` log lines.
